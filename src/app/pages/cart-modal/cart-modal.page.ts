...
// Variable para contador de cards-list page
 ...
 removeProduct(id_detalles_pedidos: Number) {
  ...
  getTotal() {
    const total = this.cartDetails.reduce((suma, item)=> suma + item.precio,0);
    return total;
  } 
  ...
  async comment(id_detalles_pedidos: Number) {
    ...
    let alert = await this.alertCtrl.create({
      ...
      inputs: [{name: 'Comentario', type:'textarea', value: this.cartDetail.comentario}],
      cssClass: 'redtext',
      buttons: [{text: 'Aceptar', cssClass: 'accept-button', handler: data => {this.cartDetail.comentario = data.Comentario; 
